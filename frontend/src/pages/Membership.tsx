...
import membershipData from '../mock/membershipData.json';

const Membership: React.FC = () => {
    const [data] = useState(membershipData);

    /* 
    // Method to fetch real data
    const fetchMembershipData = async () => {
        try {
            const response = await fetch('http://localhost:5120/api/membership/me');
            ...
            <div className="space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {data.availablePackages.map((pkg) => {
                        const isCurrent = pkg.packageId === data.userMembership.packageId;
                        return (
