...
const StatCard: React.FC<{
    ...
    <div className={`relative overflow-hidden rounded-2xl bg-white border border-amber-100 p-6 group hover:border-amber-300 transition-all duration-300 shadow-sm`}>
        <div className={`absolute inset-0 opacity-0 group-hover:opacity-10 transition-opacity duration-300 ${glow}`} />
        <div className="relative z-10">
            ...
            <div className="text-3xl font-black text-amber-950 mb-1">
                {loading ? (
                    <div className="h-8 w-20 bg-amber-50 rounded-lg animate-pulse" />
                ) : (
                    value
                )}
            </div>
            <p className="text-sm font-semibold text-amber-900/40 uppercase tracking-wider">{label}</p>
        </div>
    </div>
);
...
const AdminDashboard: React.FC = () => {
    ...
    const statCards = [
        ...
        },
        {
            ...
            color: 'bg-purple-50',
            glow: 'bg-gradient-to-br from-purple-500/10 to-transparent',
        },
        {
            ...
            color: 'bg-rose-50',
            glow: 'bg-gradient-to-br from-rose-500/10 to-transparent',
        },
        {
            label: 'System Health',
            value: 'Good',
            ...
            color: 'bg-teal-50',
            glow: 'bg-gradient-to-br from-teal-500/10 to-transparent',
        },
    ];
        ...
        <div className="p-8 max-w-7xl mx-auto space-y-8">
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-amber-900/40 text-sm mt-1">
                ...
                <button
                    onClick={fetchStats}
                    disabled={loading}
                    className="flex items-center gap-2 px-4 py-2 bg-amber-600 hover:bg-amber-500 disabled:opacity-50 text-white text-sm font-bold rounded-xl transition-all shadow-lg shadow-amber-600/20"
                >
            ...
            <div className="p-6 rounded-2xl bg-white border border-amber-100 shadow-sm">
                <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="text-amber-950 font-bold">System Status</h2>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm font-medium">
                    {['AuthService', 'ReservationService', 'MembershipService', 'LoggerService'].map((service) => (
                        <div key={service} className="flex items-center gap-2 text-amber-900/60">
