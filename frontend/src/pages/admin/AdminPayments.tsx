...
const AdminPayments: React.FC = () => {
    ...
    const statusBadge = (s: string) => {
        ...
        if (s === 'Failed') return 'bg-rose-100 text-rose-600';
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-6">
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-2xl font-black text-neutral-900">Payments</h1>
            ...
            <input
                className="w-full bg-white border border-neutral-200 rounded-xl px-4 py-2.5 text-neutral-900 text-sm placeholder-neutral-400 focus:outline-none focus:border-amber-500 transition-colors"
                            ...
                            {loading ? (
                                ...
                                filtered.map((p) => (
                                    <tr key={p.id} className="hover:bg-neutral-50 transition-colors">
                                        <td className="px-5 py-4 font-mono text-xs text-neutral-500">{p.id?.slice(0, 8)}…</td>
                                        <td className="px-5 py-4 text-neutral-900 font-semibold">{p.username || p.userId?.slice(0, 8) + '…'}</td>
                                        <td className="px-5 py-4 text-neutral-300">{p.membershipTypeName || '—'}</td>
                                        <td className="px-5 py-4 text-emerald-600 font-bold">${p.amount?.toFixed(2)} {p.currency}</td>
