...
const navItems: NavItem[] = [
    { label: 'Check-In', path: '/receptionist', icon: <ClipboardCheck className="w-5 h-5" /> },
    { label: 'Membership', path: '/receptionist/membership', icon: <ShieldCheck className="w-5 h-5" /> },
    { label: 'Schedule Training', path: '/receptionist/schedule', icon: <CalendarPlus className="w-5 h-5" /> },
    { label: 'Record Payment', path: '/receptionist/payment', icon: <CreditCard className="w-5 h-5" /> },
];
...
const ReceptionistLayout: React.FC = () => {
    ...
    return (
        <div className="min-h-screen flex bg-neutral-950">
            {/* Sidebar */}
            <aside
                className={`relative flex flex-col bg-neutral-900 border-r border-white/5 transition-all duration-300 ease-in-out ${collapsed ? 'w-20' : 'w-64'
                    }`}
            >
                {/* Logo */}
                <div className="flex items-center gap-3 px-5 py-6 border-b border-white/5">
                    ...
                    {!collapsed && (
                        <div className="overflow-hidden">
                            ...
                            <p className="text-xs font-bold text-amber-400 tracking-widest uppercase">Receptionist</p>
                        </div>
                    )}
                </div>
